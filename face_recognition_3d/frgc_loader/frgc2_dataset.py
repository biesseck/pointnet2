...
def pc_normalize(pc):
    if np.any(np.isnan(pc)):
        print 'frgc2_dataset.py: pc_normalize(): nan found in pc before normalizing!'
        sys.exit(0)
    l = pc.shape[0]
    centroid = np.mean(pc, axis=0)
    pc = pc - centroid
    ...
    pc = pc / m
    if np.any(np.isnan(pc)):
        print 'frgc2_dataset.py: pc_normalize(): nan found in pc after normalizing!'
...
class FRGCv2_Dataset():
    ...
    def _augment_batch_data(self, batch_data):
        ...
        jittered_data = provider.random_scale_point_cloud(rotated_data[:,:,0:3])   # original
        # jittered_data = provider.random_scale_point_cloud(rotated_data[:,:,0:6])     # Bernardo

        jittered_data = provider.shift_point_cloud(jittered_data)
        jittered_data = provider.jitter_point_cloud(jittered_data)
        
        rotated_data[:,:,0:3] = jittered_data    # original
        # rotated_data[:,:,0:6] = jittered_data      # Bernardo
    ...
    def _get_item(self, index):
        if index in self.cache:
            ...
            print 'frgc2_dataset.py: _get_item(): loading ', fn
            ...
            print 'point_set[1000]:', point_set[1000]
            if True in (point_set == -999999.0):
                print 'frgc2_dataset.py: _get_item(): value -999999.0 found!!!'
                sys.exit(0)
...
if __name__ == '__main__':
    ...
    # ps_batch, cls_batch = d.next_batch(True)   # original
    ps_batch, cls_batch = d.next_batch(False)     # Bernardo
